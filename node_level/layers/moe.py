...
from node_level.common.tensor import Tensor, Slice, Concat, get_tensor
from node_level.common.compute_graph import reset_compute_graph

from layers import Multicast, Barrier, Unicast, Sum
from workload import get_moe_gate_model
from config import SystemConfig
from stats import NodeStats
from utils import hash_string, dtype_to_byte
...
class MoE:
    ...
    def forward_dispatch_alltoall(self, x, stats):
        ...
        x_all = Tensor(x.uid + f"_dispatch", self.dist_info.rank, [self.global_bsz, seqlen, hidden_dim])
    ...
    def forward_dispatch_multicast(self, x, stats):
        ...
        for batch_id in batch_ids:
            ...
            if self.dist_info.is_dp_master():
                ...
                if len(dst_nodes) > 0:
                    print(f"{self.dist_info.rank}->{dst_nodes}")
    ...
    def forward_compute_experts(self, x, stats):
        ...
        if self.shared_expert:
            ...
            x_for_shared = Concat(
                ...
                axis=0, 
                uid=concat_uid).forward(stats=stats)
            exp_outs["shared"] = self.shared_expert.forward(x_for_shared, stats=stats)
    ...
    def forward_combine_alltoall(self, exp_outs, stats):
        ...
        x_all = Tensor(self.uid + f"_combine", self.dist_info.rank, [self.global_bsz*(self.num_experts_per_tok+self.n_shared_experts), seqlen, hidden_dim])
    ...
    def routings_summary(self):
        ...
        for batch_id in range(global_bsz):
            ...
            print(f"Sample {batch_id} is mapped to expert {mapped_to_expert_ids}. These experts reside in nodes {mapped_to_nodes}. Nearest shared expert reside in {shared_expert_node_id}. Sample already exists in nodes {dp_attn_cluster}. It should be sent to {send_to}")
        ...
        for node_id in range(self.dist_info.num_nodes):
            batch_ids = [batch_id for batch_id, shared_expert_node_id in self.dist_info.batch_to_shared_exp.items() if shared_expert_node_id == node_id]
            if len(batch_ids) > 0:
                print(f"Shared expert on node {node_id} processes {len(batch_ids)} samples: {batch_ids}")
...
if __name__ == "__main__":
    ...
    for rank in [0, 1, 2, 3, 4, 5, 6, 7]:
        ...
        # assert expected_footprint == op_mem_foot, f"Expected memory_footprint {expected_footprint}, got {op_mem_foot}"
