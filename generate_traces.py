...
def tile_gemm(layer_id, dims, tile_sizes):
    ...
    for m, pM in enumerate(range(0, M, Tm)):
        ...
        for k, pK in enumerate(range(0, K, Tk)):
            tiled_M = min(Tm, M - pM)
            tiled_K = min(Tk, K - pK)
            input_tiles[m][k] = Tile("{}_input_{}_{}".format(layer_id, m,k), [tiled_M, tiled_K], prec="fp16")
    ...
    for k, pK in enumerate(range(0, K, Tk)):
        ...
        for n, pN in enumerate(range(0, N, Tn)):
            tiled_N = min(Tn, N - pN)
            tiled_K = min(Tk, K - pK)
            weight_tiles[k][n] = Tile("{}_weight_{}_{}".format(layer_id, k,n), [tiled_K, tiled_N], prec="fp16")
    ...
    for m, pM in enumerate(range(0, M, Tm)):
        ...
        for n, pN in enumerate(range(0, N, Tn)):
            tiled_M = min(Tm, M - pM)
            tiled_N = min(Tn, N - pN)
            out_tiles[m][n] = Tile("{}_out_{}_{}".format(layer_id,m,n), [tiled_M, tiled_N], prec="fp16")
    ...
    for b, pB in enumerate(range(0, B, Tb)):
        ...
        for m, pM in enumerate(range(0, M, Tm)):
            ...
            for k, pK in enumerate(range(0, K, Tk)):
                ...
                tiled_M = min(Tm, M - pM)
                tiled_K = min(Tk, K - pK)
                input_tiles[b][m][k] = Tile("{}_input_{}_{}_{}".format(layer_id, b,m,k), [tiled_B, tiled_M, tiled_K], prec="fp16")
    ...
    for b, pB in enumerate(range(0, B, Tb)):
        ...
        for k, pK in enumerate(range(0, K, Tk)):
            ...
            for n, pN in enumerate(range(0, N, Tn)):
                ...
                tiled_N = min(Tn, N - pN)
                tiled_K = min(Tk, K - pK)
                weight_tiles[b][k][n] = Tile("{}_weight_{}_{}_{}".format(layer_id, b,k,n), [tiled_B, tiled_K, tiled_N], prec="fp16")
    ...
    for b, pB in enumerate(range(0, B, Tb)):
        ...
        for m, pM in enumerate(range(0, M, Tm)):
            ...
            for n, pN in enumerate(range(0, N, Tn)):
                ...
                tiled_M = min(Tm, M - pM)
                tiled_N = min(Tn, N - pN)
                out_tiles[b][m][n] = Tile("{}_out_{}_{}_{}".format(layer_id,b,m,n), [tiled_B, tiled_M, tiled_N], prec="fp16")
...
def tile_attention(layer_id, q_dims, kv_dims, pe_dims, q_tile_size, kv_tile_size):
    ...
    for b, pB in enumerate(range(0, B, Tb)):
        ...
        for h, pH in enumerate(range(0, H, Th)):
            tiled_B = min(Tb, B - pB)
            tiled_H = min(Th, H - pH)
            q_tiles[b][h] = Tile("{}_q_{}_{}".format(layer_id, b, h), [tiled_B, 1, tiled_H, D+D_pe], prec="fp16")
            out_tiles[b][h] = Tile("{}_out_{}_{}".format(layer_id, b, h), [tiled_B, 1, tiled_H, D], prec="fp16")
    ...
    for b, pB in enumerate(range(0, B, Tb)):
        ...
        for s, pS in enumerate(range(0, S_kv, Ts_kv)):
            ...
            kv_tiles[b][s] = Tile("{}_kv_{}_{}".format(layer_id, b, s), [tiled_B, tiled_S, D+D_pe], prec="fp16")
...
def map_tilegemmops(tile_ops, cores):
    num_cores = len(cores)
...
def map_tilegroupgemmops(tile_ops, cores):
    num_cores = len(cores)
...
def map_tileattnops(tile_ops, cores):
    num_cores = len(cores)
...
if __name__=="__main__":
    ...
    for node_id in range(num_nodes):
        ...
        num_cores = 24
        num_mem_banks = num_cores
        assert num_cores == num_mem_banks, "Does not support num_cores != num_mem_banks yet."

        banks = {}
        for i in range(num_mem_banks):
            banks[i] = MemoryBank(i)
        ...
        for row in data:
            ...
            if row["operation"] == "Linear":
                ...
                map_gemm_tiles(input_tiles, weight_tiles, banks)
                map_tilegemmops(tile_ops, cores)
                ...
                map_group_gemm_tiles(input_tiles, weight_tiles, banks)
                map_tilegroupgemmops(tile_ops, cores)
                ...
                map_attn_tiles(q_tiles, kv_tiles, banks)
                map_tileattnops(tile_ops, cores)
        ...
        for core_id in range(num_cores):
            traces = cores[core_id].generate_traces()

            for trace in traces:
                logging.info("Core {}: {}".format(core_id, trace))
