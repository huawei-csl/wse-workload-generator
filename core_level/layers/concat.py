...
class Concat:
    def __init__(self, uid, node_id, axis, input_dims, graph, prec) -> None:
        ...
        assert -len(input0_dims) <= axis < len(input0_dims), "Concat operation {} on node {} has invalid axis {} for input_dims {}.".format(uid, node_id, axis, input0_dims)
        if axis < 0:
            axis += len(input0_dims)
        ...
        self.output_dims = deepcopy(input0_dims)
        self.output_dims[axis] = input0_dims[axis] + input1_dims[axis]

        out_map = self._remap(self.input_tensor0.memory_map, self.input_tensor1.memory_map)
...
if __name__=="__main__":
    ...
    for node_id in range(wafer.num_nodes):
        ...
        ops[node_id][op_id] = {
            "type": "Concat",
            "inputs": [f"{node_id}:input_tensor0", f"{node_id}:input_tensor1"],
