                    ...
                    for k, pK in enumerate(range(0, K, Tk)):
                        ...
                        for n, pN in enumerate(range(0, N, Tn)):
                            ...
                            partial_sum_tensor = Tensor(
                                uid=f"{self.node_id}:{self.uid}_partial_sum_{b}_{m}_{k}_{n}",
        ...
        for b in self.reduce_ops:
            for m in self.reduce_ops[b]:
                for n in self.reduce_ops[b][m]:
                    cores_in_group = []
                    for k in self.tile_ops[b][m]:
                        c = self.tile_ops[b][m][k][n].mapped_core
                        if c not in cores_in_group:
                            cores_in_group.append(c)
                    reduce_mem = self.out_tiles[b][m][n].get_physical_address()
                    ...
                    for core in cores_in_group:
                        Barrier(f"{self.uid}_barrier_reduce_{b}_{m}_{n}",
