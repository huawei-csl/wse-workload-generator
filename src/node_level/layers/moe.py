...
from src.node_level.layers.alltoallv import AllToAllv

from src.node_level.common.tensor import Tensor, Slice, Concat, View, get_tensor
...
from src.node_level.layers.dist import get_dist_manager
...
class MoE:
    def __init__(self, uid, hidden_size, moe_intermediate_size, num_experts_per_tok, n_experts, n_shared_experts, dist_info, dtype) -> None:
        ...
        if self.rank in self.dist_info.shared_expert_ranks:
    ...
    def forward_dispatch_allgather(self, x, stats):
        ...
        # batch ids processed by this node
        batch_mapping = self.dist_info.get_batch_mapping_by_node()
        batch_ids_by_rank = {dst_id: [batch_id for batch_id in batch_mapping if batch_mapping[batch_id] == dst_id] for dst_id in range(self.dist_info.num_nodes)}

        # batch ids processed by this DP cluster
        batch_ids_by_dp_rank = self.dist_info.get_local_batchids("attn", self.rank)
        assert x.dims[0] == len(batch_ids_by_dp_rank), "Batch size of x should be equal to the number of batch ids processed by this DP cluster" 
        
        x_dispatch = []
        for batch_id in batch_ids_by_rank[self.rank]:
            minibatch_offset = batch_ids_by_dp_rank[0]
            x_slice = Slice(x, [batch_id-minibatch_offset], axis=0, uid=x.uid + f"_slice{batch_id}").forward(stats=stats)
            x_dispatch.append(x_slice)

        if len(x_dispatch) > 0:
            x_dispatch = Concat(x_dispatch, axis=0, uid=self.uid+"_dispatch_concat").forward(stats=stats)
        else:
            x_dispatch = Tensor(f"{x.uid}_empty_dispatch_tensor", self.dist_info.rank, [0, seqlen, hidden_size])

        out_ag = get_dist_manager().allgather(
            uid=self.uid+"_ag_disp",
            x=x_dispatch,
            vector_sizes=[len(batch_ids_by_rank[dst_id]) for dst_id in batch_ids_by_rank],
            dst_nodes=list(range(self.dist_info.num_nodes)),
        ...
        for e in local_experts:
            ...
            for batch_id, seq_id in recv_batch_ids:
                src_id = batch_mapping[batch_id]
                b_offset = batch_ids_by_rank[src_id][0]
                
                x_slice = Slice(out_ag[src_id], [batch_id-b_offset], axis=0, uid=x.uid + f"_expert{e}_slice{batch_id}.{seq_id}").forward(stats=stats)
            ...
            if len(recv_batch_ids) > 0:
                concat_uid = f"{self.uid}_dispatch_recv_concat_expert{e}_" + hash_string("_".join([f"{batch_id}.{seq_id}" for batch_id, seq_id in recv_batch_ids]))
                x_recv[e] = Concat(
                    recv_buff,
                    axis=0,
    ...
    def forward_dispatch_alltoall(self, x, stats):
        ...
        # batch ids processed by this DP cluster
        dp_local_batch_ids = self.dist_info.get_local_batchids("attn", self.rank)
        ...
        for dst_node_id in send_matrix[self.rank]:
            ...
            for batch_id, seq_id in send_matrix[self.rank][dst_node_id]:
                ...
                minibatch_offset = dp_local_batch_ids[0]
                x_slice = Slice(x, [batch_id-minibatch_offset], axis=0, uid=x.uid + f"_slice{batch_id}.{seq_id}_dst{dst_node_id}").forward(stats=stats)
                x_slice = Slice(x_slice, [seq_id], axis=1, uid=x_slice.uid + f"_seq{seq_id}").forward(stats=stats)
            ...
            if len(x_send_buff[dst_node_id]) > 0:
                x_send_buff_concat = Concat(
                    x_send_buff[dst_node_id],
                    axis=0,
                    uid=self.uid + "_dispatch_send_concat_" + str(dst_node_id)
                ).forward(stats=stats)
                x_send_buff_concats.append(x_send_buff_concat)
            else:
                x_send_buff_concats.append(
                    Tensor(
                        f"{self.uid}_empty_tensor_{dst_node_id}", 
                        self.dist_info.rank, 
                        [0, 1, hidden_dim]
                    )
                )
        ...
        alltoall_dispatch = AllToAllv(
            ...
            input_split = send_split,
            output_split = recv_split,
            comm_group=self.dist_info.ffn_comm_groups["ep"],
            dtype = self.dtype
        )
        alltoall_dispatch.forward(x_send_buff_concats, stats=stats)

        x_recv_buff = []
        for src_node_id in range(self.dist_info.num_nodes):
            buff_size = list(x.dims)
            buff_size[0] = len(send_matrix[src_node_id][self.rank])
            buff_size[1] = 1
            buff = Tensor(f"{self.uid}_a2a_disp_{src_node_id}", self.dist_info.rank, buff_size)
            x_recv_buff.append(
                View(buff, [buff.dims[0]*buff.dims[1], 1, buff.dims[2]]).forward(stats=stats)
            )
            ...
            if len(recv_batch_ids) > 0:
                for batch_id, seq_id in recv_batch_ids:
                    ...
                    x_recv_slice = Slice(
                        ...
                        [ind],
                        axis=0,
                        uid=x_recv_buff[src_id].uid + f"_expert{e}_slice{batch_id}.{seq_id}"
                    ).forward(stats=stats)
                    x_recv[e].append(x_recv_slice)
                x_recv[e] = Concat(
                    x_recv[e],
                    axis=0,
    ...
    def forward_dispatch_multicast(self, x, stats):
        ...
        for e in local_experts:
            ...
            if len(recv_batch_ids[e]) > 0:
                concat_uid = f"{self.uid}_dispatch_recv_concat_expert{e}_" + hash_string("_".join([f"{batch_id}.{seq_id}" for batch_id, seq_id in recv_batch_ids[e]]))
                x_recv[e] = Concat(
                    [Tensor(f"{x.uid}_slice{batch_id}.{seq_id}", self.dist_info.rank, [1, 1, hidden_dim]) for batch_id, seq_id in recv_batch_ids[e]],
                    axis=0,
                    uid=concat_uid).forward(stats=stats)
    ...
    def forward_combine_allgather(self, seqlen, exp_outs, stats):
        '''
        ...
        if len(exp_outs) > 0:
            ...
            x_local = Tensor(f"{self.uid}_empty_combine_concat", self.dist_info.rank, [0, seqlen, self.hidden_size])

        out_indices = {}
        src_buff = {src_id: [] for src_id in range(self.dist_info.num_nodes)}
        for expert_id in list(range(self.n_experts)):
            exp_node = self.expertid_to_node[expert_id]
            batch_ids = self.get_batchids_by_expert(seqlen, expert_id)
            for batch_id, seq_id in batch_ids:
                src_buff[exp_node] += [(batch_id, seq_id, expert_id)]

                if (batch_id, seq_id) not in out_indices:
                    out_indices[(batch_id, seq_id)] = []
                out_indices[(batch_id, seq_id)] += [(expert_id, exp_node, len(src_buff[exp_node])-1)]

        for src_id in range(self.dist_info.num_nodes):
            src_buff[src_id] += [(batch_id, seq_id, "shared") for seq_id in range(seqlen) for batch_id, mapped_shared in self.dist_info.batch_to_shared_exp.items() if src_id == mapped_shared]

            for batch_id, mapped_shared in self.dist_info.batch_to_shared_exp.items():
                if src_id == mapped_shared:
                    for seq_id in range(seqlen):
        ...
        out_ag = get_dist_manager().allgather(
            ...
            x=x_local,
            vector_sizes=[len(src_buff[src_id]) for src_id in src_buff],
            dst_nodes=list(range(self.dist_info.num_nodes)),
            dist_info=self.dist_info,
            dtype=self.dtype,
            stats=stats
        ...
        out_tensor = {}
        for batch_id in batch_ids_by_dp_rank:
            for seq_id in range(seqlen):
                _sum_inputs = []
                for exp_id, exp_node, out_ind in out_indices[(batch_id, seq_id)]:
                    x_slice = Slice(
                        out_ag[exp_node],
                        [out_ind],
                        axis=0,
                        uid=out_ag[exp_node].uid + f"_slice_exp{exp_id}_{batch_id}.{seq_id}"
                    ).forward(stats=stats)
                    _sum_inputs.append(x_slice)
                out_tensor[(batch_id, seq_id)] = Concat(
                    _sum_inputs,
                    axis=0,
                    uid=f"{self.uid}_combine_sum_concat_{batch_id}.{seq_id}"
                ).forward(stats=stats)
        return out_tensor


    def forward_combine_alltoall(self, seqlen, exp_outs, stats):
        if len(exp_outs) == 0:
            return 
        
        send_matrix = self.dist_info.get_combine_comm_matrix(self.uid, get_moe_gate_model(), self.n_experts, seqlen)

        x_send_buff = []
        for dst_node in send_matrix[self.rank]:
            if len(send_matrix[self.rank][dst_node]) > 0:
                exp_outs_to_send = []
                for batch_id, seq_id, expert_id in send_matrix[self.rank][dst_node]:
                    exp_input_batch_ids = self.get_batchids_by_expert(seqlen, expert_id)
                    assert len(exp_input_batch_ids) == exp_outs[expert_id].dims[0], "Length of exp_input_batch_ids {} does not match the batch dimension of exp_outs {}".format(len(exp_input_batch_ids), exp_outs[expert_id].dims[0])
                    ...
                    x_slice = Slice(
                        ...
                        [minibatch_ind], 
                        axis=0, 
                        uid=exp_outs[expert_id].uid + f"_slice{batch_id}.{seq_id}"
                    ).forward(stats=stats)
                    exp_outs_to_send.append(
                        x_slice
                    )
                concat_uid = self.uid + "_gather_unicast_concat_" + hash_string("_".join([f"{batch_id}.{seq_id}e{expert_id}" for batch_id, seq_id, expert_id in send_matrix[self.rank][dst_node]]))
                x_send_buff.append(
                    Concat(
                        exp_outs_to_send, 
                        axis=0,
        ...
        alltoall_combine.forward(x_send_buff, stats=stats)

    def forward_combine_multicast(self, seqlen, exp_outs, stats):
        if len(exp_outs) == 0:
            return 
        ...
        for e in self.experts:
            ...
            batch_ids = self.get_batchids_by_expert(seqlen, e)
 
            dst_nodes = [self.dist_info.get_batchid_to_dispatch_src(batch_id) for batch_id, seq_id in batch_ids]

            for (batch_id, seq_id), dst_node in zip(batch_ids, dst_nodes):
                batchid_dst[dst_node].append((e, batch_id, seq_id))

        # # sort the batchids for each dst_node, first by the expert_id then by batch_id
        # for dst_node in batchid_dst:
        #     batchid_dst[dst_node] = sorted(batchid_dst[dst_node], key=lambda x: (x[2], x[1], x[0]) ) # sort by batch_id

        if self.shared_expert:
            batch_ids = self.get_batchids_by_expert(seqlen, "shared")

            dst_nodes = [self.dist_info.get_batchid_to_dispatch_src(batch_id) for batch_id, seq_id in batch_ids]

            for (batch_id, seq_id), dst_node in zip(batch_ids, dst_nodes):
                batchid_dst[dst_node].append(("shared", batch_id, seq_id))
        ...
        for dst_node in batchid_dst:
            ...
            if len(batchid_dst[dst_node]) > 0:
                exp_outs_to_send = [] 
                for expert_id, batch_id, seq_id in batchid_dst[dst_node]:
                    exp_input_batch_ids = self.get_batchids_by_expert(seqlen, expert_id)
                ...
                concat_uid = self.uid + "_gather_unicast_concat_" + hash_string("_".join([f"{batch_id}.{seq_id}e{expert_id}" for expert_id, batch_id, seq_id in batchid_dst[dst_node]]))
    ...
    def forward_combine_allgather_add(self, seqlen, exp_outs, stats):
        out_sums = []
        for batch_id, seq_id in exp_outs:
            x_recv = exp_outs[(batch_id, seq_id)]
        ...
        batch_ids = self.dist_info.get_local_batchids("attn", self.rank)
        ...
        #             recv_buffer = Tensor(f"{self.uid}_combine_concat_ag_{src_id}", self.dist_info.rank, [len(exp_input_batch_ids), 1, self.hidden_size])
        ...
        #             )
        #             cnt += len(exp_input_batch_ids)

        #         src_id = self.dist_info.batch_to_shared_exp[batch_id]
        #         recv_buffer = Tensor(f"{self.uid}_combine_concat_ag_{src_id}", self.dist_info.rank, [len(batch_ids), 1, self.hidden_size])

        #         exp_outs_to_recv.append(
        #             Slice(recv_buffer, [batch_id-batch_ids[0]], axis=0, uid=recv_buffer.uid + f"_shared_slice{batch_id}.{seq_id}").forward(stats=stats)
        #         )

        #         assert len(exp_outs_to_recv) == self.num_experts_per_tok + self.n_shared_experts

        #         concat_uid = f"{self.uid}_combine_add_concat_batchid{batch_id}.{seq_id}"
        #         x_recv = Concat(
        #                 exp_outs_to_recv, 
        #                 axis=0,
        #                 uid=concat_uid).forward(stats=stats)
        ...
        if len(out_sums) > 0:
            ...
            out = View(out, [len(batch_ids), seqlen, self.hidden_size]).forward(stats=stats)
        else:
            out = Tensor(
        ...
        assert out.dims[0] == len(batch_ids) and out.dims[1] == seqlen 

        return out

    def forward_combine_alltoall_add(self, seqlen, exp_outs, stats):
        batch_ids = self.dist_info.get_batch_dist_within_dp()

        recv_buffer = {}
        for src_id in self.combine_traffic:
            if self.dist_info.rank in self.combine_traffic[src_id]:
                recv_buffer[src_id] = Tensor(f"{self.uid}_combine_a2a_{src_id}", self.dist_info.rank, [len(self.combine_traffic[src_id][self.dist_info.rank]), 1, self.hidden_size])
            ...
            for seq_id in range(seqlen):
                ...
                for expert_id in expert_ids:
                    ...
                    if src_id == self.dist_info.rank:
                        exp_input_batch_ids = self.get_batchids_by_expert(seqlen, expert_id)
                        buff_ind = exp_input_batch_ids.index((batch_id, seq_id))
                        uid = f"{exp_outs[expert_id].uid}_node{src_id}_slice{buff_ind}.{seq_id}"
                                             
                        assert exp_outs[expert_id].dims[1] == 1
                        exp_outs_to_recv.append(
                        ...
                        exp_outs_to_recv.append(
                            Slice(recv_buffer[src_id], [buff_ind], axis=0, uid=recv_buffer[src_id].uid + f"_slice{buff_ind}.{seq_id}").forward(stats=stats)
                        )
                
                src_id = self.dist_info.batch_to_shared_exp[batch_id]
                if src_id == self.dist_info.rank:
                    exp_input_batch_ids = self.get_batchids_by_expert(seqlen, "shared")
                    assert len(exp_input_batch_ids) == exp_outs["shared"].dims[0], "Length of exp_input_batch_ids {} does not match the batch dimension of exp_outs {}".format(len(exp_input_batch_ids), exp_outs["shared"].dims[0])
                    buff_ind = exp_input_batch_ids.index((batch_id, seq_id))
                    uid = f"{exp_outs['shared'].uid}_node{src_id}_slice{buff_ind}.{seq_id}"
                    
                    assert exp_outs["shared"].dims[1] == 1
                    exp_outs_to_recv.append(
    ...
    def forward_combine_multicast_add(self, seqlen, exp_outs, stats):
        batch_ids = self.dist_info.get_batch_dist_within_dp()
    ...
    def forward_distribute_dp(self, x, stats):
        ...
        local_batchids = self.dist_info.get_local_batchids("attn", self.rank)
    ...
    def forward(self, x, stats): 
        ...
        if self.dist_info.ep > 1:
            ...
            elif self.dist_info.moe_comm == "alltoall":
            ...
            if self.dist_info.moe_comm == "allgather":
                ag_out = self.forward_combine_allgather(seqlen, exp_outs, stats=stats)
                
                out_tensor = self.forward_combine_allgather_add(seqlen, ag_out, stats=stats)

            elif self.dist_info.moe_comm == "alltoall":
                self.calc_combine_traffic()

                self.forward_combine_alltoall(seqlen, exp_outs, stats=stats)

                out_tensor = self.forward_combine_alltoall_add(seqlen, exp_outs, stats=stats)
                ...
                self.forward_combine_multicast(seqlen, exp_outs, stats=stats)
    ...
    def routings_summary(self, seqlen):
        ...
        for expert_id in range(self.n_experts):
            ...
            if node_id not in outputs:
                outputs[node_id] = []
            outputs[node_id] += [(expert_id, batch_id, seq_id) for batch_id, seq_id in batch_ids]
        ...
        for node_id in range(self.dist_info.num_nodes):
            ...
            if len(batch_ids) > 0:
                ...
                if node_id not in outputs:
                    outputs[node_id] = []
                outputs[node_id] += [("shared", batch_id, seq_id) for batch_id, seq_id in batch_ids]
